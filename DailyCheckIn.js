...
export class dailycheckin extends plugin {
    ...
    async checkIn(e) {
        ...
        if (e.group_id) {
            ...
            sign_group_user.group_id = e.group_id
            let mySignInInfo = await redis.get("Lycoris:checkIn:" + sign_group_user.user_qq + ":" + sign_group_user.group_id)
            if (mySignInInfo == null) {
                ...
                if (t_money) {
                    ...
                    await redis.set("Lycoris:td_ci:" + td_ci_date + ':' + sign_group_user.user_qq + ":" + sign_group_user.group_id, tci, { EX: 3600 * 24 })
                    await redis.set("Lycoris:checkIn:" + sign_group_user.user_qq + ":" + sign_group_user.group_id, sgu, { EX: 3600 * 24 * 90 })
                    mySignInInfo = await redis.get("Lycoris:checkIn:" + sign_group_user.user_qq + ":" + sign_group_user.group_id)
                ...
                let td_ci_date = await this.formatNowDate()
                let last_ci_info_json = await redis.get("Lycoris:td_ci:" + td_ci_date + ':' + sign_group_user.user_qq + ":" + sign_group_user.group_id)
                let last_ci_info = JSON.parse(last_ci_info_json)
                logger.mark('????xxx' + last_ci_info)
                if (last_ci_info) {
            ...
            let data = {
                tplFile: './plugins/example/resources/signin/signin.html',
                dz: ml,
